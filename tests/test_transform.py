...
from src.rllib2onnx import restore, SimplePolicyConfigAdapter, rllib_to_onnx
...
class TestTransform(unittest.TestCase):
    ...
    def test_filter(self):
        self._test({"observation_filter": "MeanStdFilter"})

    def _test(self, extra_cfg: Optional[Dict[str, Any]] = None):
        # Train on CartPole for one iteration
        trainer = ppo.PPOTrainer(
            config={"framework": "tf", "num_workers": 0, **(extra_cfg or {})},
            env="CartPole-v0",
        )
        trainer.train()
        ...
        restored_trainer = restore(
            ...
            env="CartPole-v0",
            config_adapter_cls=SimplePolicyConfigAdapter
        )
